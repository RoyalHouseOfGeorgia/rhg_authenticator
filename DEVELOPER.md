...
└── __tests__/
    ...
    ├── fixtures/
    │   └── vectors.json        # Cross-language test vectors (regenerated by go/testdata/gen_vectors.go; byte-identical to go/testdata/vectors.json)
    ├── canonical.test.ts
    ├── base64url.test.ts
    ├── credential.test.ts
...
- 392 tests total (10 test files)

### Regenerating cross-language test vectors

`go/testdata/gen_vectors.go` is the single source-of-truth generator for the
cross-language test vectors. Run it from the `go/` directory:

```bash
cd go && go run ./testdata/gen_vectors.go > /tmp/vectors.json && mv /tmp/vectors.json testdata/vectors.json
```

Then copy the regenerated output to the TypeScript fixture location so both
copies stay byte-identical (still in `go/` after the command above):

```bash
cp testdata/vectors.json ../src/__tests__/fixtures/vectors.json
```
