...
const (
	...
	registryPath     = "verify/keys/registry.json"
...
// filePath is not URL-escaped — it comes from the registryPath constant, not user input.
...
func (c *Client) CreateRegistryPR(ctx context.Context, content []byte, title string) (PRResult, error) {
	...
	branchName, err := c.createBranchWithRetry(ctx, mainSHA)
	if err != nil {
		return PRResult{}, err
	}
	...
	// 4. Get file SHA for the registry on main.
	fileSHA, err := c.getContents(ctx, registryPath, "main")
	if err != nil {
		cleanup()
		return PRResult{}, fmt.Errorf("getting registry file SHA: %w", err)
	}

	// 5. Update contents with one retry on 409 (stale SHA).
	if err := c.updateContentsWithRetry(ctx, branchName, content, fileSHA, title); err != nil {
		cleanup()
		return PRResult{}, err
	}

	// 6. Create PR.
	pr, err := c.createPR(ctx, branchName, "main", title, "Registry update submitted via RHG Authenticator")
	if err != nil {
		cleanup()
...
func (c *Client) createBranchWithRetry(ctx context.Context, sha string) (string, error) {
	for attempt := range maxBranchRetries {
		suffix, err := core.RandomHex(16)
		if err != nil {
			return "", fmt.Errorf("generating branch suffix: %w", err)
		}
		branchName := "registry-update-" + time.Now().UTC().Format("20060102T150405Z") + "-" + suffix
...
func (c *Client) updateContentsWithRetry(ctx context.Context, branch string, content []byte, fileSHA, message string) error {
	err := c.updateContents(ctx, registryPath, branch, content, fileSHA, message)
	if err == nil {
		return nil
	}
	...
	newSHA, fetchErr := c.getContents(ctx, registryPath, branch)
	if fetchErr != nil {
		return fmt.Errorf("re-fetching registry SHA after 409: %w", fetchErr)
	}

	if retryErr := c.updateContents(ctx, registryPath, branch, content, newSHA, message); retryErr != nil {
